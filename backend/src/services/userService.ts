...
export function createUserService({ db }: { db: Db }) {
  async function me(req: Request, res: Response) {
    try {
      ...
      console.error('[me]', e);
      if (!res.headersSent) return res.status(500).json({ ok: false, message: e?.message || 'failed' });
    }
  }
  ...
  async function updateMe(req: Request, res: Response) {
    try {
      ...
      if (!res.headersSent) return res.status(500).json({ ok: false, message: e?.message || 'update failed' });
    }
  }
