...
export function createCategoryService() {
  function categories(req: Request, res: Response) {
    const keywordThumbMap: Array<{ keywords: string[]; url: string }> = [
    ...
    const cdnThumb = (name: string) => {
      const target = keywordThumbMap.find((row) => row.keywords.some((kw) => String(name || '').includes(kw)));
      return target?.url || fallbackThumb;
    };
    const leaf = (id: string, name: string) => ({ id, name, thumbnail: cdnThumb(name) });
    const level2 = (name: string, children: any[]) => ({ name, children });
    const level1 = (name: string, children: any[]) => ({ name, children });
    const data = [
      level1('方便速食', [
