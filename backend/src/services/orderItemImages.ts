...
export function extractProductIdFromOrderItem(item: Record<string, unknown>): number | null {
...
export function hydrateOrderItemsWithProduct(db: Db, items: unknown[]): Record<string, unknown>[] {
  ...
  const stmt = db.prepare(`SELECT id, title, image, unit FROM products WHERE id = ?`);
  return items.map((raw) => {
    const item = raw as Record<string, unknown>;
    const pid = extractProductIdFromOrderItem(item);
    if (pid == null) return { ...item };
    const row = stmt.get(pid) as { title: string; image: string | null; unit: string | null } | undefined;
    if (!row) return { ...item };
    const next = { ...item };
    if (existingImageSrc(item) === '' && row.image) {
      next.primaryImage = row.image;
      next.image = row.image;
      next.thumb = row.image;
    }
    const u = String(row.unit ?? '').trim();
    if (!next.unit || String(next.unit).trim() === '') next.unit = u || '件';
