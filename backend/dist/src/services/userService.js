...
export function createUserService({ db }) {
    function me(req, res) {
        ...
        const user = db.prepare(`SELECT id, openid, nickName, avatarUrl, gender, phoneNumber FROM users WHERE id = ?`).get(userId);
    ...
    function updateMe(req, res) {
       ...
       WHERE id = ?`).run(nickName ?? null, avatarUrl ?? null, gender ?? null, phoneNumber ?? null, userId);
        const meRow = db.prepare(`SELECT id, openid, nickName, avatarUrl, gender, phoneNumber FROM users WHERE id = ?`).get(userId);
