...
export function createAdminService({ db, uploadsDir }) {
    ...
    function adminUpdatePassword(req, res) {
        ...
        const adminId = req.admin?.id;
        const row = db.prepare(`SELECT id, password FROM admins WHERE id = ?`).get(adminId);
        if (!row || row.password !== parsed.data.currentPassword)
            return res.status(401).json({ ok: false, message: '当前密码错误' });
        db.prepare(`UPDATE admins SET password = ?, sessionToken = NULL, updatedAt = datetime('now') WHERE id = ?`).run(parsed.data.newPassword, adminId);
        return res.json({ ok: true, data: { ok: true } });
    }
    function adminUpdateUsername(req, res) {
        const schema = z.object({
