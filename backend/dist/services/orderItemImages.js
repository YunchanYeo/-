...
export function hydrateOrderItemsWithProduct(db, items) {
    ...
    return items.map((raw) => {
        ...
        if (existingImageSrc(item) !== '' || pid == null)
            return { ...item };
        ...
        if (!next.goodsName || String(next.goodsName).trim() === '')
            next.goodsName = row.title;
