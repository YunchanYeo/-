...
import { fetchCartGroupData, updateLocalCartItemSelection, updateLocalCartStoreSelection, updateLocalCartItemQuantity, deleteLocalCartItem, clearLocalInvalidCartItems, } from '../../services/cart/cart';
Page({
    data: {
        cartGroupData: null,
    },
    ...
    onShow() {
        ...
        // 탭 전환으로 돌아왔을 때도 최신 장바구니를 다시 읽어야
        // 상품목록에서 방금 추가한 항목이 즉시 보입니다.
        this.refreshData();
    },
    onLoad() {
        this.refreshData();
    ...
    refreshData() {
        return this.getCartGroupData().then((res) => {
            ...
            if (!Array.isArray(cartGroupData.invalidGoodItems)) {
            ...
            const totalGoodsInStores = cartGroupData.storeGoods.reduce((sum, store) => {
                const groupCount = (store.promotionGoodsList || []).reduce((groupSum, group) => groupSum + (group.goodsPromotionList || []).length, 0);
                return sum + groupCount + (store.shortageGoodsList || []).length;
            }, 0);
            if (totalGoodsInStores > 0 || cartGroupData.invalidGoodItems.length > 0) {
                isEmpty = false;
            }
    ...
    selectGoodsService({ spuId, skuId, isSelected }) {
        updateLocalCartItemSelection({ spuId, skuId, isSelected });
        return Promise.resolve();
    },
    // 全选门店
    // 注：实际场景时应该调用接口更改选中状态
    selectStoreService({ storeId, isSelected }) {
        updateLocalCartStoreSelection({ storeId, isSelected });
        return Promise.resolve();
    },
    // 加购数量变更
    // 注：实际场景时应该调用接口
    ...
    deleteGoodsService({ spuId, skuId }) {
        deleteLocalCartItem({ spuId, skuId });
        return Promise.resolve();
    },
    // 清空失效商品
    // 注：实际场景时应该调用接口
    clearInvalidGoodsService() {
        clearLocalInvalidCartItems();
        return Promise.resolve();
    },
    onGoodsSelect(e) {
