...
Page({
    ...
    handleOptionsParams(options, couponList) {
        ...
        this.userAddressReq = userAddressReq || this.userAddressReq || null;
        this.setData({
            userAddressReq: this.userAddressReq,
            userAddress: this.userAddressReq || this.data.userAddress || null,
        });
        const params = {
            goodsRequestList,
            storeInfoList,
        ...
        fetchSettleDetail(params).then((res) => {
            ...
            const settlePayload = res.data || res;
            this.initData(settlePayload);
            const nextGoods = deriveGoodsRequestListFromSettleDetail(settlePayload);
            if (nextGoods.length > 0) {
                this.goodsRequestList = nextGoods;
            }
        }, () => {
            //接口异常处理
            this.handleError();
    ...
    initData(resData) {
        ...
        const fallbackAddress = this.userAddressReq || this.data.userAddressReq || this.data.userAddress || null;
        const finalAddress = resData?.userAddress || fallbackAddress;
        this.userAddressReq = finalAddress;
        this.setData({
            userAddressReq: finalAddress,
    ...
    isInvalidOrder(data) {
        ...
        if ((data.limitGoodsList && data.limitGoodsList.length > 0) ||
    ...
    onGoodsNumChange(e) {
        const { detail: { value }, currentTarget: { dataset: { goods }, }, } = e;
